...
export function SubmitFriendLinkPage({
  ...
  return (
    <div className="flex flex-col gap-4 w-full">
      {/* Header */}
      <div
        className="cuckoo-card-base p-6 md:p-8 relative overflow-hidden flex flex-col items-center justify-center min-h-48 cuckoo-onload-animation bg-linear-to-br from-(--cuckoo-primary)/5 to-transparent"
        style={{ animationDelay: "150ms" }}
      >
        <h1 className="text-3xl font-bold cuckoo-text-90 mb-4 z-10 transition-colors">
          {m.friend_link_submit_title()}
        </h1>
        <p className="cuckoo-text-50 text-center max-w-xl z-10 transition-colors">
          {m.friend_link_submit_desc()}
        </p>
        <Link
          to="/friend-links"
          className="mt-6 z-10 flex items-center gap-2 text-sm text-(--cuckoo-primary) hover:underline transition-all"
        >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-4">
        {/* Form Section */}
        <div
          className="lg:col-span-3 cuckoo-card-base p-6 md:p-8 cuckoo-onload-animation"
          style={{ animationDelay: "300ms" }}
        >
          <h2 className="text-xl font-bold cuckoo-text-90 mb-6 flex items-center gap-2 transition-colors">
            <PlusCircle className="w-5 h-5 text-(--cuckoo-primary)" />
          ...
          <form onSubmit={form.handleSubmit} className="space-y-5">
            <div>
              <label
                htmlFor="siteName"
                className="block text-sm font-medium cuckoo-text-75 mb-1.5 transition-colors"
              >
            ...
            <div>
              ...
              <input
                ...
                type="url"
                className="w-full px-4 py-2.5 rounded-xl border border-(--cuckoo-input-border) bg-(--cuckoo-input-bg) focus:outline-none focus:ring-2 focus:ring-(--cuckoo-primary)/50 focus:border-transparent transition-all"
                placeholder={m.friend_link_placeholder_logo_url_default()}
              />
              {form.errors.logoUrl && (
                <p className="mt-1.5 text-sm text-red-500">
            ...
            <div className="pt-4">
              <button
                ...
                className="w-full flex justify-center py-3 px-4 border border-transparent rounded-xl shadow-sm text-sm font-bold cuckoo-btn-primary active:scale-95 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-(--cuckoo-primary) disabled:opacity-50 disabled:cursor-not-allowed transition-all"
              >
                {form.isSubmitting ? (
        ...
        {/* My Links Section */}
        <div
          className="lg:col-span-2 cuckoo-card-base p-6 md:p-8 cuckoo-onload-animation self-start"
          style={{ animationDelay: "450ms" }}
        >
          <h2 className="text-xl font-bold cuckoo-text-90 mb-6 transition-colors">
          ...
          <div className="space-y-4">
            {myLinks.length > 0 ? (
              myLinks.map((link) => (
                <div
                  key={link.id}
                  className="p-4 rounded-2xl bg-black/5 dark:bg-white/5 border border-black/5 dark:border-white/5 transition-all hover:border-(--cuckoo-primary)/30"
                >
                  <div className="flex items-start justify-between gap-2 mb-2">
                    <h3 className="font-bold cuckoo-text-90 truncate transition-colors">
                      {link.siteName}
                    </h3>
                  ...
                  <a
                    ...
                    className="text-xs cuckoo-text-50 hover:text-(--cuckoo-primary) transition-colors truncate block mb-3"
                  >
                    {link.siteUrl}
                  </a>
                  ...
                  <div className="text-[10px] cuckoo-text-30 text-right mt-2 transition-colors">
              ...
              <div className="py-12 text-center cuckoo-text-30 transition-colors">
