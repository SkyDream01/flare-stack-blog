...
export function PostCard({
  ...
  return (
    <div
      className={`fuwari-card-base fuwari-post-card flex flex-col w-full rounded-(--fuwari-radius-large) overflow-hidden relative ${
    ...
    >
      ...
      <div className="pl-5 sm:pl-6 md:pl-9 pr-5 sm:pr-6 pt-5 sm:pt-6 md:pt-7 pb-5 sm:pb-6 relative w-full md:pr-24">
        {/* Badge */}
        {(pinned || popular) && (
          ...
          {tagNames.length > 0 && (
            <div className="flex min-w-0 items-center">
              ...
              <div className="flex flex-row flex-wrap items-center gap-x-1.5">
                {tagNames.map((name, i) => (
                  <span key={name} className="flex min-w-0 items-center">
                    {i > 0 && (
                    ...
                    <Link
                      to="/posts"
                      search={{ tagName: name }}
                      className="fuwari-expand-animation min-w-0 wrap-anywhere rounded-md px-1.5 py-2 text-sm font-medium hover:text-(--fuwari-primary)"
                    >
                      {name}
                    </Link>
                  </span>
        ...
        {/* Description */}
        <div
          className={`fuwari-text-75 wrap-anywhere leading-relaxed empty:hidden ${
            pinned
        ...
        {/* Read time and Views */}
        <div className="text-sm fuwari-text-50 flex flex-wrap items-center gap-x-4 gap-y-2 [&_svg]:shrink-0">
