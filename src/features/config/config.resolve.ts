...
export function resolveSiteConfig(
  ...
  const defaultBackground = config?.site?.theme?.default?.background;
    ...
    theme: {
      default: {
        ...
        background: defaultBackground
          ? {
              homeImage: defaultBackground.homeImage ?? "",
              globalImage: defaultBackground.globalImage ?? "",
              light: {
                opacity: defaultBackground.light?.opacity ?? 0.15,
              },
              dark: {
                opacity: defaultBackground.dark?.opacity ?? 0.1,
              },
              backdropBlur: defaultBackground.backdropBlur ?? 8,
              transitionDuration: defaultBackground.transitionDuration ?? 600,
            }
          : undefined,
      },
      fuwari: {
